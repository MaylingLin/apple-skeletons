...
Apple Skeletons 是一个面向苹果三维结构重建的研究代码仓库。该项目旨在从至少两张 RGB 图像中恢复苹果的三维点云，并结合苹果参数化几何模型，实现形状拟合、不可见区域补全与表面重建，最终生成较完整的苹果三维模型。

基于重建得到的三维模型，可以获取目标物的整体几何形状信息，并形成对其表面空间结构的显式表达。在此基础上，可进一步开展表面精细化分析，例如苹果表面瑕疵检测、瑕疵区域面积计算、表面结构分析等。同时，该三维模型也可用于数字孪生建模与虚拟重建，为后续可视化分析、仿真计算以及机器人采摘过程中的目标识别、空间定位与路径规划等任务提供基础数据支撑。

本代码仓的核心技术思想并不依赖苹果这一特定对象，而是基于“理论几何模型约束三维重建过程”的通用框架。通过替换相应的理论几何模型，该方法也可迁移至梨、桃、柑橘等具有相对规则几何特征的水果三维重建任务，体现出一定的跨类别适应性与泛化能力。
